...
function MenuIcon({ open }: { open: boolean }) {
  return (
    <span aria-hidden="true" className="relative block h-5 w-5">
      <span
        className={`absolute left-0 top-1/2 h-px w-5 bg-gray-950 transition-all duration-300 ${
...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-[100] w-full border-b border-gray-200 bg-white/95 backdrop-blur-xl">
      <Container>
        {/* DESKTOP / MAIN HEADER */}
        <div className="flex h-20 w-full items-center justify-between">
              ...
              {navigation.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      aria-hidden="true"
                      className={`absolute bottom-1 left-3 right-3 h-px origin-center bg-gray-950 transition-transform duration-300 ${
                        active
                          ? "scale-x-100"
          ...
          <Link
            href="/contact"
            className="group hidden shrink-0 items-center gap-3 rounded-full bg-gray-950 px-5 py-3 font-[var(--font-primary)] text-button text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-gray-800 hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-950 focus-visible:ring-offset-2 sm:inline-flex"
          >
            ...
            >
              <div className="flex flex-col">
                ...
                <Link
                  href="/contact"
                  onClick={closeMenu}
                  className="group mt-5 flex min-h-12 items-center justify-center gap-3 rounded-full bg-gray-950 px-6 py-3 font-[var(--font-primary)] text-button text-white transition-all duration-300 hover:bg-gray-800"
                >
                  <span>Contact Us</span>
